...
class TestBooksCollector:
    ...
    @pytest.mark.parametrize('book', ['Мальчик c пальчик', 'Барабашка']) #параметризация проверяет добавление новых книг в список
    ...
    def test_set_book_genre_valid_name(self, books_collector):# проверка добавления новых книг с корректным названием и присвоение книгам жанра
        ...
        assert books_collector.get_book_genre('Агата не дремлет') == 'Детективы'
    ...
    def test_get_books_with_specific_genre_when_valid_genre(self, books_collector):# проверка что выводится список по жанрам. Добавляем книу, задаем жанр, проверяем жанр и что список книг ему соответствует.
        ...
        books_with_genre = books_collector.get_books_with_specific_genre('Мультфильмы')
        assert isinstance(books_with_genre, list)
        assert 'Буратино' in books_with_genre
    ...
    # Проверяем, что метод возвращает словарь, а не None или другой тип
        result = books_collector.get_books_genre()
        assert result is not None  
        assert isinstance(result, dict)  
    # Добавляем книги и проверяем, что все еще возвращается словарь
